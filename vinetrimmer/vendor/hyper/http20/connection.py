...
"""
import vinetrimmer.vendor.h2.connection
import vinetrimmer.vendor.h2.events
import vinetrimmer.vendor.h2.settings
...
class HTTP20Connection(object):
    ...
    def __init_state(self):
        ...
        objects themselves.
        """
        self._conn = _LockedObject(vinetrimmer.vendor.h2.connection.H2Connection())
    ...
    def _send_preamble(self):
        ...
        with self._conn as conn:
            ...
            conn.update_settings(
                {vinetrimmer.vendor.h2.settings.ENABLE_PUSH: int(self._enable_push)}
            )
        self._send_outstanding_data()
    ...
    def _single_read(self):
        ...
        for event in events:
            if isinstance(event, vinetrimmer.vendor.h2.events.DataReceived):
                self._adjust_receive_window(event.flow_controlled_length)
                self.streams[event.stream_id].receive_data(event)
            elif isinstance(event, vinetrimmer.vendor.h2.events.PushedStreamReceived):
            ...
            elif isinstance(event, vinetrimmer.vendor.h2.events.ResponseReceived):
                self.streams[event.stream_id].receive_response(event)
            elif isinstance(event, vinetrimmer.vendor.h2.events.TrailersReceived):
                self.streams[event.stream_id].receive_trailers(event)
            elif isinstance(event, vinetrimmer.vendor.h2.events.StreamEnded):
                self.streams[event.stream_id].receive_end_stream(event)
            elif isinstance(event, vinetrimmer.vendor.h2.events.StreamReset):
            ...
            elif isinstance(event, vinetrimmer.vendor.h2.events.ConnectionTerminated):
